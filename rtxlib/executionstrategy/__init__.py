...
from rtxlib.executionstrategy.PCAGaussOptimizerStrategy import start_pca_gauss_optimizer_strategy
...
def run_execution_strategy(wf):
    ...
    try:
        ...
        if wf.execution_strategy["type"] == "sequential":
            log_results(wf.outputFolder, wf.execution_strategy["knobs"][0].keys() + ["result"], append=False)
            start_sequential_strategy(wf)

        elif wf.execution_strategy["type"] == "self_optimizer":
            log_results(wf.outputFolder, wf.execution_strategy["knobs"].keys() + ["result"], append=False)
        ...
        elif wf.execution_strategy["type"] == "pca_gauss":
            log_results(wf.outputFolder, wf.execution_strategy["knobs"].keys() + ["result"], append=False)
            start_pca_gauss_optimizer_strategy(wf)
