...
def evaluator(resultState, wf):
    return resultState["varianceWarmupTime"]


def state_initializer(state, wf):
    state["varianceWarmupTime"] = 999999999
    return state


def primary_data_reducer(state, newData, wf):
    if (state["varianceWarmupTime"] > newData["varianceWarmupTime"]):
        state["varianceWarmupTime"] = newData["varianceWarmupTime"]
    return state


currentConfiguration = {
    "scenarioIdentifier": "lowVariance",
    "scenarioCommand": " -t 1 -n 100 ",
